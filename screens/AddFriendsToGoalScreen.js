...
export default function AddFriendsToGoalScreen({ navigation, route }) {
  ...
  const handleContinue = async () => {
    ...
    if (!goalListId) {
      Alert.alert('Error', 'Goal list ID is missing.');
      navigation.goBack();
      return;
    }
    ...
    setLoading(true);
    try {
      // Save selected friends as participants in Supabase
      ...
      Alert.alert('Success', `${selectedFriends.length} friend(s) added successfully!`);
      navigation.goBack();
    } catch (error) {
